...
constexpr const char* kDefaultUris = "default=mysql://root@127.0.0.1:3306";
...
std::vector<SqlSpec> parse_uris(const std::string& spec) {
...
public:
    ...
    caf::actor spawn(caf::actor_system& sys,
        ...
        std::string uris = caf::get_or(sys.config(), "caf-plugin-system.mysql-uris",
        ...
        return sys.spawn([logger, uris, pool_size](caf::event_based_actor* self) -> caf::behavior {
            ...
            return caf::behavior{business.or_else(plugin_lifecycle(self, PluginLifecycleHooks{
                .on_init = [=](caf::actor, const std::string&) {
                    LOG_INFO_SELF(self, "MySqlPlugin initialized, uris={}", uris);
                    launch_workers();
                    selfcheck();
                },
