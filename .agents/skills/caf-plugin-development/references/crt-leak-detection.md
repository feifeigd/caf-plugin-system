...
- **unload 后必须 ExitProcess(0) 的场景**：仅当不先 clear meta 时——否则 CAF 全局
  meta 注册表析构调用已卸载 DLL 的函数指针 → 0xC0000005。**推荐路径**：先
  `clear_global_meta_objects()` 再 unload + 正常 return（实测无崩，保留静态析构 +
  dump 完整触发，见下方替代方案节）。
- **ExitProcess 前必须 flush**：重定向到文件时 stdout 全缓冲，ExitProcess
  ...
  ```
- **ExitProcess 语义**：不执行进程级 CRT 清理链（atexit/stdio flush/静态析构；
  CRT dump 是 atexit 注册 → ExitProcess 路径不会触发 dump），但会通知已加载 DLL 的
  DLL_PROCESS_DETACH——本项目动态 CRT（VS2022+UCRT 22621）实测 stdout 缓冲仍丢，以实测为准。
