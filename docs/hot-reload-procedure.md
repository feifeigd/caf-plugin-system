# 热更新流程 Hot-Reload Procedure

> 项目：caf-plugin-system · 本文档描述插件 DLL 热更新的完整流程与约束
> 对接接口：quiesce / save_state / send_exit / hot_reload_atom / register_atom / resume_atom / retired_ / plugin_lib_pool
...
每节点一个 OpsActor（`src/core/cluster/ops_actor.*`），提供两个入口：
**本地控制台**（stdin 交互）与 **master 远程热更**（跨节点）。协议为内核
固定类型（message_tags.def 261-265，与集群协议同级，非插件 envelope）。

## 消息协议（message_tags.def）

| ID | 类型 | 说明 |
|---|---|---|
| 261 | `remote_reload_atom` | master→节点：远程热更（载荷 `reload_request`） |
| 262 | `reload_request` | 双模式载荷：`dll_bytes` 非空 = 字节流推送；否则 `dll_path` 已就位 |
| 263 | `reload_result` | 回执：`{ok, message}` |
| 264 | `console_cmd_atom` | stdin 线程 → 本机 OpsActor（命令字符串） |
| 265 | `list_plugins_atom` | 查询本机插件列表 → `vector<string>` |
...
list                              # 本机插件列表
nodes                             # 集群拓扑（仅 master）
reload <plugin> <dll_path>        # 本机热更（所有节点）
reload-node <node> <plugin> <p>   # 远程热更单节点（仅 master）
reload-nodes <glob> <plugin> <p>  # 远程热更匹配节点（仅 master；* ? 通配）
reload-all <plugin> <p>           # 远程热更全部节点（仅 master；排除自身）
quit                              # 优雅关机
```

- **双模式自动切换**：`p` 是 master 本地文件 → 读字节流推送（节点落盘
  `updates/<plugin>/<ts>-<pid>/` 新路径，PID 后缀防同机多实例撞路径）；
  否则按"已就位路径"直接传（DLL 已通过 CI/CD/共享盘到达节点本地）。
- **批量语义**：逐节点并行发起（异步链互不阻塞），共享计数汇总
  `N OK, M FAILED (of T)`；单节点失败不影响其他节点。

## 寻址与安全

- 节点把 ops actor 放入 CAF registry（名 `"ops"`）并随 `exported_actors`
  上报 master；master 侧 `node_resolve(node, "ops")` → middleman
  `connect_atom` → `remote_lookup("ops")` → `request(remote_reload)`。
- 不校验跨节点 sender（CAF 同机多进程 node_id 相同，sender 会失效）；
  安全边界 = middleman 端口可达性 + 集群互信。`reload-node*` 仅 master
  进程可用（需要 master_registry 句柄）。

## 验证示例（四进程实测全绿）

```
master 控制台: reload-all BusinessPlugin ./updates/business_plugin_v2.dll
[Ops] batch reload 'BusinessPlugin' -> 3 node(s)
[Ops] pushing 270848 bytes of business_plugin_v2.dll to node 'worker-a'
[Ops]   worker-a: OK - reloaded
[Ops]   worker-b: OK - reloaded
[Ops]   worker-c: OK - reloaded
[Ops] batch done: 3 OK, 0 FAILED (of 3)
```

自动化验证后门：`--test-remote-reload=<node>,<plugin>,<path>`（逗号分隔 =
单节点；`reload-* ` 前缀 = 完整命令原样发）。
