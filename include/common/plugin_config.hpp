...
//       X(caf::settings, uris, {}) \
//       XC(int, pool_size, 2, PLUGIN_CONF_CLI)   // XC = 显式标记
...
//     vector<T>/caf::settings（dictionary，嵌套块天然支持）...
...
//                                         （敏感配置推荐：URI 密码不进
//                                          命令行/ps/任务管理器）
//   XC(Type, Name, Def, PLUGIN_CONF_CLI)—— conf + CLI 双通道
//   按字段粒度控制，互不影响。
//
// CLI 注册的接线：带 PLUGIN_CONF_CLI 标记的字段会生成注册条目
// （PLUGIN_FIELD_REG_PLUGIN_CONF_CLI），由 register_plugin_config()
// 统一执行——该函数由内核 declare_config 钩子（plugin_manager 在
// spawn 前回调插件）调用，Phase 4 接线；当前为空实现占位。
// ------------------------------------------------------------------
...
// -- X-macro 字段处理 ------------------------------------------------------
// X = 3 参（默认只 conf），XC = 4 参（显式标记）。
// DECL/LOAD 用 ... 兼容两种形式；REG_X 固定 3 参、REG_XC 固定 4 参，
// 完全避开 MSVC 传统预处理器的空 __VA_ARGS__ / NARG 坑。
...
// X 条目：默认只 conf，不注册 CLI
#define PLUGIN_FIELD_REG_X(Type, Name, Def) /* conf only */

// XC 条目：按显式标记分支
#define PLUGIN_FIELD_REG_XC(Type, Name, Def, Mode)                             \
  PLUGIN_FIELD_REG_##Mode(Type, Name, Def)
#define PLUGIN_FIELD_REG_PLUGIN_CONF_ONLY(Type, Name, Def) /* conf only */
...
#define PLUGIN_CONFIG(Fields)                                                  \
  namespace {                                                                  \
  struct PluginConfig {                                                        \
    Fields(PLUGIN_FIELD_DECL, PLUGIN_FIELD_DECL)                               \
  };                                                                           \
  inline PluginConfig load_plugin_config(const caf::actor_system_config& cfg) { \
    PluginConfig c;                                                            \
    Fields(PLUGIN_FIELD_LOAD, PLUGIN_FIELD_LOAD)                               \
    return c;                                                                  \
  }                                                                            \
  inline void register_plugin_config(caf::actor_system_config& cfg) {          \
    /* Phase 4 由内核 declare_config 钩子调用；当前空实现占位。            */ \
    Fields(PLUGIN_FIELD_REG_X, PLUGIN_FIELD_REG_XC)                            \
  }                                                                            \
  } // namespace
