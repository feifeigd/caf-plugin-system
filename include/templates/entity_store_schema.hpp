...
// EntityStore 的服务端 Schema 与读写路由（存储后端无关）。
// 业务请求中的实体名/字段名只用于查白名单，实际存储标识符全部来自
// 这份受信配置。后端专属约束由各自的配置层进一步校验。
...
namespace caf_plugin_system::entity_store::schema {
...
public:
    ...
    explicit schema_catalog(bool allow_binary)
        : allow_binary_(allow_binary) {}
    ...
    bool add_store(store_schema store, std::string& error) {
        ...
        for (const auto& [name, entity] : store.entities) {
            ...
            if (!validate_entity(entity, error, allow_binary_)) {
    ...
    static bool validate_entity_schema(const entity_schema& entity,
        ...
        return validate_entity(entity, error, allow_binary);
    }
    ...
    parse(const caf::settings& stores, const std::string& default_read_service,
          ...
          bool allow_binary = false) {
        schema_catalog result{allow_binary};
...
private:
    ...
    static bool validate_entity(const entity_schema& entity,
                                std::string& error, bool allow_binary) {
        ...
        for (const auto& key : entity.keys) {
            if (!validate_field(key, error, allow_binary))
                return false;
        ...
        for (const auto& field : entity.fields) {
            if (!validate_field(field, error, allow_binary))
                return false;
    ...
    static bool validate_field(const field_schema& field,
        ...
        if ((field.kind == value_kind::bytes && !allow_binary)
